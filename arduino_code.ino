//code modified from: https://curiousscientist.tech/blog/as5600-magnetic-position-encoder
...
float rpmInterval = 10; // RPM is calculated every 0.2 seconds
...
float radPerSec = 0; //radians per second


float radPerSecSquared = 0;
...
{    
    ...
    //calculateAngularVelocityAndAcceleration(); 
    Serial.print(millis());
    Serial.print(","); 
    Serial.println(totalAngle*3.1415926/180); 
    // Serial.print(",");
    // Serial.println(radPerSecSquared); 
    delay(1); //wait a little - adjust it for "better resolution"
...
void calculateAngularVelocityAndAcceleration()
{
  ...
  previousVelocity = radPerSec; 

  if (timerdiff > rpmInterval)
  {

    //rpmValue = (60000.0/rpmInterval) * (totalAngle - recentTotalAngle)/360.0;
    rpmValue = ((totalAngle-recentTotalAngle)/360.0)/(timerdiff/60000);
    radPerSec = rpmValue * 0.10472; 
    radPerSecSquared = (radPerSec-previousVelocity)/(timerdiff/60000);
